...
export const useValidation = (validator: Validator) => {
    ...
    const onChangeInput = (ev: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        setErrors({
            ...errors,
            [name]: formData[name].validations.find((validation: ValidationRule) => !validation.check(inputValue))?.errorMessage,
        });
    };
    ...
    const onChangeValue = (name: string, value: string) => {
        ...
        setErrors({
            ...errors,
            [name]: formData[name].validations.find((validation: ValidationRule) => !validation.check(value))?.errorMessage,
        });
    };
    ...
    const handleCheckFormErrors = (fields?: string[]) => {
        ...
        keysToValidate.forEach((key) => {
            ...
            const validation = formData[key].validations.find((validation: ValidationRule) => !validation.check(values[key]));
            updatedErrors[key] = validation ? validation.errorMessage : "";
        });

        setErrors(updatedErrors);

        return keysToValidate.some((key) => typeof updatedErrors[key] === "string" && updatedErrors[key].trim() !== "");
    };
