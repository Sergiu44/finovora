...
import { ArrowDownLeft, ArrowUpDown, ArrowUpRight, Trash } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const AccordionTransactionGroupedByDate = ({
  ...
  const formatSummaryAmount = (amount: number) => {
    ...
    return `${amount >= 0 ? "+" : ""}${formattedAmount} ${currencyCode ?? ""}`;
  };
        ...
        {Object.entries(groupedTransactions).map(([dateKey, summary]) => {
          ...
          return (
            <AccordionItem key={dateKey} value={dateKey}>
              <AccordionTrigger
                type="button"
                className="group w-full flex items-center justify-between gap-4 px-4 py-3 text-left transition-colors hover:bg-primary/10"
              >
                <div>
                ...
                <div className="flex items-center gap-4 text-xs font-semibold">
                  <span className="text-green-600 group-data-[state=open]:text-green-400">
                    Income: {formatSummaryAmount(summary.totalIncome)}
                  </span>
                  <span className="text-red-600 group-data-[state=open]:text-red-400">
                  ...
                  {summary.transactions.map((transaction) => (
                    ...
                    >
                      <div
                        ...
                        className="select-none grow flex items-center justify-between p-3 bg-white rounded-base border border-gray-200 hover:border-primary/30 hover:bg-white transition-colors"
                      >
