...
export const getAccountsCardFormatHandler = catchErrors(async (req: Request, res: Response) => {
  const accounts = await Account.findAll({
    where: {
      userId: req.userId,
    },
...
export const getAccountHandler = catchErrors(async (req: Request, res: Response) => {
  const account = await Account.findOne({
    ...
    order: [[{ model: Transaction, as: "transactions" }, "transactionDate", "ASC"]], // Order by most recent first
  });
  ...
  const expenseCurrentMonth =
    currentMonthTransactions
      ?.filter((transaction) => transaction.transactionType.name === "Expense")
      .reduce((acc, transaction) => acc + transaction.amount, 0) || 0;
  const incomePercentage = incomeCurrentMonth ? ((incomeCurrentMonth - incomeLastMonth) / incomeLastMonth) * 100 : null;
  ...
  return res
    .status(200)
    .json({ ...account?.toJSON(), transactions: currentMonthTransactions, incomePercentage, expensePercentage });
});

export const createAccountHandler = catchErrors(async (req: Request, res: Response) => {
  const { name, accountTypeId, description, currencyId, gradientId, balance, type } = req.body;
  ...
  const newAccount = await Account.create({
    ...
    currencyId,
    balance,
    ...(isDefaultGradient(gradient) ? { defaultGradientId: gradient.id } : { userGradientId: gradient.id }),
  });
